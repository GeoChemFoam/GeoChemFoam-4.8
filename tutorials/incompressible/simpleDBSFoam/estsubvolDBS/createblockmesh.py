...
for i in range (0,nLevel):
    ...
    os.system('rm -f system/cellSetDict')
    f=open("system/cellSetDict",'a')
    f.seek(0) #get to the first position
    f.write("FoamFile"+'\n')
    f.write("{"+'\n')
    f.write("    version     2.0;"+'\n')
    ...
    f.write("    object      cellSetDict;"+'\n')
    f.write("}"+'\n')
    f.write(""+'\n')
    f.write("name refinementRegion;"+'\n')
    f.write("action new;"+'\n')
    f.write("topoSetSources"+'\n')
    f.write("("+'\n')
    nlist=len(micro_por);
    ...
    for line in Lines:
      ...
      if (wbool==1):
          epsVal=float(ls)
          if (epsVal<0.35) and (epsVal>0.0001):
